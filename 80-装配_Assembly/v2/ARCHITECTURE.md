...
## 闭合解(elevated platform home)

旧渲染"平躺/张翼"的真因不是数值,而是**架构**:接收器被当成与伺服同高的零件。
实情是接收器是一块**被抬起的运动平台**:

- 固件 home 反推主销距伺服轴 162.48mm;在 identity(z=0)接收器距伺服仅 ~75mm,
  175mm 长杆在该处无法闭合 —— 于是杆只能向外张成"翅膀"。
- 以固件 reach 常数为几何约束,对两个俯仰耳做最小二乘,解出平台需沿 +Z **抬升 153.5mm**
  才能让接收器销点落到"距伺服 162.48mm"的 home 球面上。
- 抬升后:6 条杆从真实曲柄销**向上汇聚**到被抬起的接收器,蓝色接收套筒居中、黑色套筒架在顶端,
  整机呈 SR6 真机的高耸运动平台形态(`v2_platform.png` / `v2_platform_vs_ref.png`),不再平躺。

杆的摆放遵循真实自由度:曲柄销处是球铰,杆绕销自由摆;故把杆的孔轴朝**最近的平台销点**定向
(`rot_between`),杆自然竖起汇聚到平台,而非张翼。`platform_home.build_platform()` 输出整机位姿。
