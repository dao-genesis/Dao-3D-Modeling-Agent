...
_CURATED: Dict[str, Dict[str, Any]] = {
    ...
    "asm.dof": {"desc": "自由度报告：按已记录配合(align/stack/coaxial/fix)逐零件给出剩余自由度与配合图；"
                        "含接地链诊断(floating)/过约束(redundant)；interference=true 联动干涉并标注 mated。",
                "params": _schema({})},
    ...
    "doc.export": {"desc": "按扩展名把指定对象(或全部带形对象)导出为 STEP/IGES/BREP/STL/OBJ/DXF/SVG…",
                   "params": _schema({"path": _STR, "object": _STR,
                                     "objects": {"type": "array", "items": _STR}}, ["path"])},
    "obj.expressions": {"desc": "全文档表达式普查：列出每个对象上绑定表达式引擎的全部属性。",
                        "params": _schema({})},
    "sketch.external": {"desc": "外部几何：把其他对象的边/点(如 Pad 的 Edge3)投影进草图作参考。",
                        "params": _schema({"sketch": _STR, "object": _STR, "sub": _STR},
                                         ["sketch", "object", "sub"])},
    "sketch.expression": {"desc": "给命名尺寸约束绑表达式(如 W ← Spreadsheet.L*2)；expr 空则清除。",
                          "params": _schema({"sketch": _STR, "name": _STR, "expr": _STR},
                                           ["sketch", "name"])},
    "gui.appearance": {"desc": "官方 ViewObject 外观面：读写可见性/颜色[r,g,b 0-1]/透明度 0-100/线宽/显示模式。",
                       "params": _schema({"object": _STR, "visible": _BOOL,
                                         "color": {"type": "array", "items": _NUM},
                                         "transparency": _INT, "line_width": _NUM,
                                         "display_mode": _STR}, ["object"])},
    "gui.camera": {"desc": "活视口相机：读/设位置与朝向四元数、正交/透视、聚焦对象、fit。",
                   "params": _schema({"position": {"type": "array", "items": _NUM},
                                     "orientation": {"type": "array", "items": _NUM},
                                     "projection": _STR, "focus": _STR, "fit": _BOOL})},
    "draw.dimension": {"desc": "TechDraw 真尺寸标注：type Distance/DistanceX/DistanceY/Radius/Diameter/Angle, references 如 ['Edge0'] 或 ['Vertex0','Vertex1']。",
                       "params": _schema({"view": _STR, "type": _STR,
                                         "references": {"type": "array", "items": _STR},
                                         "x": _NUM, "y": _NUM}, ["view", "references"])},
    "draw.export": {"desc": "TechDraw 图纸页按扩展名导出 SVG/DXF/PDF。",
                    "params": _schema({"page": _STR, "path": _STR}, ["path"])},
    "param.hole": {"desc": "官方 PartDesign Hole：positions 处打孔，可通孔/定深、沉头(countersink)/沉孔(counterbore)/螺纹；孔从草图面向下钻，offset 抬到料顶(=料厚)。",
                   "params": _schema({"body": _STR, "positions": {"type": "array"},
                                     "diameter": _NUM, "depth": _NUM, "through": _BOOL,
                                     "type": _STR, "cut_diameter": _NUM, "cut_depth": _NUM,
                                     "threaded": _BOOL, "plane": _STR, "offset": _NUM,
                                     "feature": _STR}, ["body", "positions"])},
    "param.draft": {"desc": "官方 PartDesign 拔模：给定 faces(0基索引)绕中性面斜 angle 度，neutral 默认底面。",
                    "params": _schema({"body": _STR, "faces": {"type": "array", "items": _INT},
                                      "angle": _NUM, "neutral": _INT, "reversed": _BOOL,
                                      "feature": _STR}, ["body", "faces"])},
    "draft.text": {"desc": "模型空间 3D 注释文字(Draft Text)。",
                   "params": _schema({"text": _STR, "position": {"type": "array", "items": _NUM},
                                     "height": _NUM, "name": _STR}, ["text"])},
    "draft.dimension": {"desc": "模型空间两点线性尺寸标注(Draft Dimension)。",
                        "params": _schema({"start": {"type": "array", "items": _NUM},
                                          "end": {"type": "array", "items": _NUM},
                                          "via": {"type": "array", "items": _NUM},
                                          "name": _STR}, ["start", "end"])},
    "draft.clone": {"desc": "参数化克隆(Draft Clone)：跟随原对象，可独立缩放/摆放。",
                    "params": _schema({"object": _STR, "scale": {}, "position": {"type": "array", "items": _NUM},
                                      "name": _STR}, ["object"])},
    # ── pref.* 偏好参数 ─────────────────────────────────────────────
